...
## Testing Guidelines
Tests use XCTest and should be named `test...` with behavior-focused descriptions, as in `testLeavingZoneResetsContinuousDelay`. Favor deterministic detector tests with explicit timestamps, face boxes, and landmarks. Add or update core tests for any change to zone scoring, activation delay, face-hold behavior, or alert state. Extend the existing lifecycle tests for delay resets, face-hold expiry, and live hand-size limits instead of duplicating their setup and assertions. Run `swift test` before opening a PR; for camera, menu-bar, preview, or packaging changes, also run the packaged app briefly and inspect the log.
